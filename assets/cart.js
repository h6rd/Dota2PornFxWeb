...
function addToCart(mod, categoryId) {
    const FORBIDDEN_CATEGORIES = ['guides', 'optimization', 'packs'];
    const SINGLE_ITEM_CATEGORIES = ['terrains', 'shaders', 'creep-deny', 'ti-bp-effects', 'emblems', 'versus-screens', 'trees', 'roshan', 'creeps', 'ancient', 'tormentor', 'towers', 'ranged-attack', 'mega-kill', 'pedestal'];
    ...
    try {
        ...
        for (const item of cart) {
            ...
            try {
                ...
                showToast(`Warning: Could not add ${item.name}`);
            }
        }
   ...
   - Для английского: -language 123`;
        ...
        try {
            ...
            if (exeResponse.ok) {
                ...
                console.warn('VPKMerge.exe not found');
            }
        } catch (error) {
            console.warn('Could not add VPKMerge.exe:', error);
        }
        ...
        showToast('Pack downloaded successfully!');

    } catch (error) {
        console.error('Pack error:', error);
        showToast('Error creating pack. Please try again.');
    } finally {
