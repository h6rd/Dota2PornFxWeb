...
// Send Mod Modal
function setupSendModModal() {
    const sendModButton = document.getElementById('sendModButton');
    const sendModModal = document.getElementById('sendModModal');
    const sendModOverlay = document.getElementById('sendModOverlay');
    const closeSendModModal = document.getElementById('closeSendModModal');
    const sendModForm = document.getElementById('sendModForm');

    if (!sendModModal || !sendModOverlay) return;

    const closeSendModWindow = () => {
        sendModModal.classList.remove('active');
        sendModOverlay.classList.remove('active');
        document.body.style.overflow = '';

        setTimeout(() => {
            const modalContent = sendModModal.querySelector('.info-modal-content');
            if (modalContent) modalContent.scrollTop = 0;
            if (sendModForm) sendModForm.reset();
            const fileLabel = sendModModal.querySelector('.file-name');
            if (fileLabel) fileLabel.textContent = 'Choose file...';
        }, 300);
    };

    sendModButton?.addEventListener('click', (e) => {
        e.preventDefault();
        e.stopPropagation();
        sendModModal.classList.add('active');
        sendModOverlay.classList.add('active');
        document.body.style.overflow = 'hidden';
    ...
    closeSendModModal?.addEventListener('click', closeSendModWindow);
    sendModOverlay.addEventListener('click', closeSendModWindow);

    document.addEventListener('keydown', (e) => {
        if (e.key === 'Escape' && sendModModal.classList.contains('active')) {
            closeSendModWindow();
        }
    });
        ...
        fileInput.addEventListener('change', (e) => {
            ...
            if (file) {
                if (file.size > MAX_FILE_SIZE) {
                    showToast(
                        'File too large. Maximum size: 50MB<br><a href="https://t.me/f4cks0ciety" target="_blank" style="color: var(--md-sys-color-primary); text-decoration: underline; font-weight: 500;">Send mod directly to Telegram</a>',
                        5000
                    );
                    fileInput.value = '';
    ...
    if (sendModForm) {
        sendModForm.addEventListener('submit', async (e) => {
            e.preventDefault();

            const submitBtn = document.getElementById('sendModSubmit');
            const submitText = submitBtn.querySelector('.submit-text');
            const originalText = submitText.textContent;
            const telegramFallback = document.getElementById('telegramFallback');

            const modTitle = document.getElementById('modTitle').value.trim();
            ...
            if (modFile.size > MAX_FILE_SIZE) {
                showToast(
                    'File too large. Maximum size: 50MB<br><a href="https://t.me/dota2pornfx" target="_blank" style="color: var(--md-sys-color-primary); text-decoration: underline; font-weight: 500;">Send mod directly to Telegram</a>',
                    5000
                );
                return;
            }
            ...
            try {
                ...
                const response = await fetch('/api/submit-mod', {
                    ...
                    body: JSON.stringify({
                        ...
                        fileSize: (modFile.size / (1024 * 1024)).toFixed(2)
                    })
                });
                ...
                if (result.success) {
                    showToast('Mod sent successfully!');
                    closeSendModWindow();
                } else {
                    if (response.status === 429) {
                        showToast(result.error, 5000);
                    } else {
                        showToast(result.error || 'Error sending mod. Please try again.');
                    }
                ...
                console.error('Error sending mod:', error);
                showToast('Error sending mod. Please try again.');
